...
# =============================================================================
# Metrics endpoint (from Version 1)
# =============================================================================
...
# =============================================================================
# API Key Middleware (from Version 1)
# =============================================================================


class TestAPIKeyMiddleware:
    @pytest.mark.asyncio
    async def test_health_exempt_without_key(self, test_client):
        """Health endpoint must not require API key."""
        response = await test_client.get("/health")
        assert response.status_code != 401

    @pytest.mark.asyncio
    async def test_query_allowed_without_key_when_disabled(self, test_client):
        response = await test_client.post("/query", json={"question": "test"})
        assert response.status_code != 401
...
class TestRequestIDs:
    ...
    @pytest.mark.asyncio
    async def test_response_has_process_time_header(self, test_client):
        response = await test_client.get("/health")
